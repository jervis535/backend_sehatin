import express from 'express';
import pool from '../utils/dblogin.js';

const router = express.Router();

// Create a new medical history
router.post('/medicalhistory', async (req, res) => {
  const { user_id, medications, medical_conditions, notes } = req.body;

  try {
    const result = await pool.query(
      `INSERT INTO medical_histories (user_id, medications, medical_conditions, notes)
       VALUES ($1, $2, $3, $4)
       RETURNING *`,
      [user_id, medications, medical_conditions, notes]
    );

    res.status(201).json(result.rows[0]);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
});

// Get all medical history
router.get('/medicalhistory', async (req, res) => {
  const userId = req.query.user_id;
  try {
    let result;

    if (userId) {
      result = await pool.query(
        'SELECT * FROM medical_histories WHERE user_id = $1',
        [userId]
      );
    } else {
      result = await pool.query('SELECT * FROM medical_histories');
    }
    res.status(200).json(result.rows);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});


// Update medical history by id
router.put('/medicalhistory/:id', async (req, res) => {
  const { medications, medical_conditions, notes } = req.body;
  const { id } = req.params;

  try {
    const result = await pool.query(
      `UPDATE medical_histories
       SET medications = $1, medical_conditions = $2, notes = $3
       WHERE id = $4
       RETURNING *`,
      [medications, medical_conditions, notes, id]
    );

    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Medical history not found' });
    }

    res.json(result.rows[0]);
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

// Delete medical history by id
router.delete('/medicalhistory/:id', async (req, res) => {
  const { id } = req.params;

  try {
    const result = await pool.query(
      'DELETE FROM medical_histories WHERE id = $1 RETURNING *',
      [id]
    );

    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'Medical history not found' });
    }

    res.json({ message: 'Medical history deleted', data: result.rows[0] });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});

export default router;
